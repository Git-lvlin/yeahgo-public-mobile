...
const Address = ({ value, onChange }) => {
  ...
  return (
    <>
      <Space>
        <Select placeholder="请选择省份" value={province?.id} style={{ width: 70, margin: 0, padding: 0 }} onChange={handleProvinceChange}>
          {provinceData.map(item => (
            <Option key={item.id}>{item.name}</Option>
          ))}
        </Select>
        {province?.id && <Select placeholder="请选择城市" value={city?.id} style={{ width: 70, margin: 0, padding: 0 }} onChange={onCityChange}>
          {cityData.map(item => (
            <Option key={item.id}>{item.name}</Option>
          ))}
        </Select>}
        {city?.id && <Select placeholder="请选择地区" value={area?.id} style={{ width: 70, margin: 0, padding: 0 }} onChange={onAreaChange}>
          {areaData.map(item => (
            <Option key={item.id}>{item.name}</Option>
          ))}
        </Select>}
